    ...
    originalRedis.RedisClient.prototype.internal_send_command = function (command_obj) {
        if (command_obj) {
            ...
            if (!cb.pubsubBound) {
                ...
                command_obj.callback = channel.bindToContext(function (err, result) {
                    const hrDuration = process.hrtime(startTime);
                    const duration = (hrDuration[0] * 1e3 + hrDuration[1]/1e6)|0;
                    channel.publish<RedisData>('redis', {duration, address, command_obj, err, result});

                    if (typeof cb === 'function') {
                        cb.apply(this, arguments);
                    }
                });
                command_obj.callback.pubsubBound = true;
            }
        }

        return originalSend.call(this, command_obj);
    }
